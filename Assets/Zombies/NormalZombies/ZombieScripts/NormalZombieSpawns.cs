...
public class NormalZombieSpawns : MonoBehaviour
{
    ...
    IEnumerator SpawnZombiesDogs()
    {
        for (int i = 0; i < _zombieCount; i++)
        {
            GameObject zombie = ZombieOP.SharedInstance.GetDogPoolObject();
            if (zombie != null)
            {
            ...
            yield return new WaitForSeconds(3); //delay the spawn between zombies so they don't spawn into each other
        }
